...
The whole table can be downloaded on the search page using the `Download` button or directly at the link below. Alternatively, filtering using the inputs on the search page allows for retrieval a modified export file.
...
The data file for the CCGD is rebuilt daily for the most updated version of gene data with source pulls from NCBI. References to Sanger data sources are updated weekly.
...
In addition, there is a description of the relevant findings for each study on the [references](http://hst-ccgd-prd-web.oit.umn.edu/references.html) page.
